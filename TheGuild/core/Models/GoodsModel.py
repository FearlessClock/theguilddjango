...
class ItemInformation(BaseTimeModel):
    ...
    def GetCurrentPrice(self):
        ...
        return int(
            self.Remap(
                int(pnoise1(x / 16.0 * octaves, octaves) * 127.0 + 128.0),
                0.0,
                250,
                self.price_min,
                self.price_max,
    ...
    def TickGoods(self):
        ...
        if secondsSinceLastUpdate > self.tick_in_seconds:
            ...
            self.tick = int(
                (datetime.now(UTC) - self.start_date).total_seconds()
                / self.tick_in_seconds
            )
            self.save()
...
class Recipe(models.Model):
    name = models.CharField(max_length=200)
    constructed_item_information = models.ForeignKey(
        ItemInformation, on_delete=models.CASCADE
    )
