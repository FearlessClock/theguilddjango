...
class HireNewEmployeeView(APIView):
    ...
    def post(self, request):  # Hire a new employee
        ...
        employee = Employee.objects.filter(
            is_assigned=False, country=workshop.character.country
        ).first()
        if employee:
            ...
            employee.save()
            return Response(
                {"success": True, "employee": employee.id}, status=status.HTTP_200_OK
            )
        else:
            return Response(
                {"success": False, "message": "No available employee found"},
...
class GiveRecipeToEmployeeView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"success": False, "employee": employeeID, "recipeID": recipeID},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"success": False, "employee": employee.id, "recipeID": recipeID},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        employee.save()
        return Response(
            {"success": True, "employee": employee.id, "recipeID": recipeID},
    ...
    def delete(self, request):
        try:
            ...
            return Response(
                {"success": False, "employee": employee.id},
                status=status.HTTP_400_BAD_REQUEST,
            )
