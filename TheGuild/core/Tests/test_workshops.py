...
class WorkshopModelTest(TestAuthBase):
    def setUp(self):
        ...
        self.workshop = Workshop.objects.create(
            id=10, building=building, character=self.char, storage=storage
        )
        Workshop_Recipe.objects.create(
            workshop=self.workshop, recipe=Recipe.objects.get(id=1), is_available=True
        )
    ...
    def test_create_workshop(self):
        ...
        building = Building.objects.create(
            country=char.country, grid_point=grid_point, type="test", name="test"
        )
        grid_point.has_building = True
        grid_point.save()
        storage = Storage.objects.create(number_of_storage_spaces=2)
        workshop = Workshop.objects.create(
            id=5, building=building, character=char, storage=storage
        )
    ...
    def test_workshop_by_country(self):
        token = Token.objects.get_or_create(
            user=User.objects.get(username="AmusedSandpaper")
        )
    ...
    def test_workshop_hire_employee(self):
        resp = self.client.post(
            "/api/workshop/hire/", data={"workshopID": 10}, format="json"
        )
    ...
    def test_workshop_fire_employee(self):
        resp = self.client.delete(
            "/api/workshop/hire/", data={"employeeID": 1}, format="json"
        )
        self.assertIs(resp.status_code, 200)
        self.assertIs(resp.json()["success"], True)
        resp = self.client.delete(
            "/api/workshop/hire/", data={"workshopID": 1}, format="json"
        )
        self.assertIs(resp.json()["success"], False)
    ...
    def test_give_employee_job(self):
        ...
        resp = self.client.post(
            "/api/workshop/give-recipe/",
            data={"employeeID": 1, "recipeID": 1},
            format="json",
        )
        self.assertIs(resp.json()["success"], True)
    ...
    def test_remove_employee_job(self):
        ...
        self.assertIs(resp.json()["success"], True)
        resp = self.client.delete(
            "/api/workshop/give-recipe/", data={"employeeID": 1}, format="json"
        )
        self.assertIs(resp.json()["success"], True)
