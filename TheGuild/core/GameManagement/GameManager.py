...
def TickCountry(countryID):
    ...
    if secondsSinceLastUpdate > country.tick_in_seconds:
        ...
        country.tick = int(
            (datetime.now(UTC) - country.start_date).total_seconds()
            / country.tick_in_seconds
        )
        country.save()
...
def UpdateWorkshop(workshop):
    ...
    if secondsSinceLastUpdate > workshop.tick_in_seconds:
        ...
        workshop.tick = int(
            (datetime.now(UTC) - workshop.start_date).total_seconds()
            / workshop.tick_in_seconds
        )
        employees = Employee.objects.filter(
            workshop_id=workshop.id, is_assigned=True
        ).values()
        recipeIdsWorkedOn = []
        for employee in employees:
            if employee["active_recipe_id"] is not None:
                workedOnRecipe = Workshop_Recipe.objects.get(
                    recipe_id=employee["active_recipe_id"], workshop_id=workshop.id
                )
                if workedOnRecipe:
                    found = False
                    for i in range(len(recipeIdsWorkedOn)):
                        if recipeIdsWorkedOn[i][0] == workedOnRecipe.id:
                            recipeIdsWorkedOn[i] = (
                                recipeIdsWorkedOn[i][0],
                                recipeIdsWorkedOn[i][1] + 1,
                            )
                            found = True
                    if not found:
        ...
        for recipeWorkedOn in recipeIdsWorkedOn:
            ...
            ticksSinceLastUpdate = int(
                (datetime.now(UTC) - recipe.last_update).total_seconds()
                / workshop.tick_in_seconds
            )
            recipe.current_progress += ticksSinceLastUpdate
            numberProducedInTime = int(
                recipe.current_progress
                / (recipe.recipe.construction_ticks / recipeWorkedOn[1])
            )
            numberReallyProduced = HowManyProduced(
                numberProducedInTime, workshop, recipe
            )
            if numberReallyProduced > 0:
                UpdateGoods(workshop, recipe, numberReallyProduced)
                if HasResources(workshop, recipe):
                    recipe.current_progress = recipe.current_progress % (
                        recipe.recipe.construction_ticks / recipeWorkedOn[1]
                    )
                else:
                    recipe.current_progress = 0
...
def UpdateGoods(workshop, workshop_recipe, amount):
    storage_goods = Storage_Goods.objects.get_or_create(
        storage_id=workshop.storage.id,
        goods_data_id=workshop_recipe.recipe.constructed_goods.id,
    )
