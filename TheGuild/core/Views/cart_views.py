...
from TheGuild.core.Serializers.cart_serializer import (
    CartSerializer,
    CartStorageSerializer,
)
...
class CartCountryView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if filter is not None:
            chars = Character.objects.filter(country_id=filter).values_list(
                "id", flat=True
            )
    ...
    def get_queryset(self):
        ...
        if workshopID is not None:
            ...
            carts = Cart.objects.filter(
                character_id=workshop.character.id,
                location_id=workshop.building.id,
                is_traveling=False,
            )
            return carts
        return None
...
class WorkshopToCartTransferView(APIView):
    ...
    def post(self, request):
        ...
        if cart_id is None or workshop_id is None:
            return Response(
                "Cart or Workshop not found", status=status.HTTP_204_NO_CONTENT
            )
        ...
        if cart.location_type.lower() != "workshop" or cart.location_id != workshop_id:
            return Response(
                "This cart is not at the correct location for this task",
                status=status.HTTP_400_BAD_REQUEST,
            )

        workshop = Workshop.objects.get(id=workshop_id)
        try:
            workshop_goods = Storage_Goods.objects.get(
                storage_id=workshop.storage.id, goods_data_id=goods_id
            )
        except Exception as error:
            return Response(
                "Could not find goods in workshop" + error,
                status=status.HTTP_400_BAD_REQUEST,
            )
        number_of_items_in_cart = Storage_Goods.objects.filter(
            storage_id=cart.storage.id
        ).count()
        cart_goods = Storage_Goods.objects.get_or_create(
            storage_id=cart.storage.id, goods_data_id=goods_id
        )
        if (
            cart_goods[1]
            and number_of_items_in_cart > cart.storage.number_of_storage_spaces
        ):
            return Response(
                "Not enough space in the cart", status=status.HTTP_400_BAD_REQUEST
            )
        elif (
            not cart_goods[1]
            and cart_goods[0].quantity + quantity > cart_goods[0].max_stack_size
        ):
            return Response(
                "Storage space too small", status=status.HTTP_400_BAD_REQUEST
            )
...
class StorageToStorageTransferView(APIView):
    ...
    def post(self, request):
        ...
        if storage_1_id == storage_2_id:
            return Response(
                "Cannot move goods to same storage", status=status.HTTP_400_BAD_REQUEST
            )

        if ItemInformation.objects.get(id=goods_id) is None:
            return Response(
                "Goods id missing or non existant", status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if storage_1 is None or storage_2 is None:
            return Response(
                "Could not find Storage models", status=status.HTTP_204_NO_CONTENT
            )
        goods_to_move = Storage_Goods.objects.get(
            storage_id=storage_1_id, goods_data_id=goods_id
        )
        if goods_to_move is None:
            return Response(
                "No resources with that id available",
                status=status.HTTP_400_BAD_REQUEST,
            )
        if goods_to_move.quantity < quantity:
            return Response(
                "Not enough resources available", status=status.HTTP_400_BAD_REQUEST
            )
        move_to_goods = Storage_Goods.objects.get_or_create(
            storage_id=storage_2_id, goods_data_id=goods_id
        )
        if (
            move_to_goods[1]
            and storage_2.number_of_storage_spaces
            <= Storage_Goods.objects.filter(storage_id=storage_2_id).count() + 1
        ):
            return Response(
                "Not enough space in the second storage",
                status=status.HTTP_400_BAD_REQUEST,
            )
        elif (
            not move_to_goods[1]
            and move_to_goods[0].quantity + quantity > move_to_goods[0].max_stack_size
        ):
            return Response(
                "Storage space too small", status=status.HTTP_400_BAD_REQUEST
            )
...
class SetCartInMotion(APIView):
    ...
    def post(self, request):
        ...
        if cartID is None:
            return Response(
                "Please add the cart ID to the path params",
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if cart.is_traveling:
            return Response(
                "Cart already in motion", status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not grid:
            return Response(
                "Grid point does not exist", status=status.HTTP_400_BAD_REQUEST
            )
        cart.UpdateCart()
        cart.target_x = x
        cart.target_y = y
        cart.travel_duration_seconds = (
            abs(x - cart.current_x)
            + abs(y - cart.current_y) * cart.travel_speed_per_block
        )
